...
function renderPage(emp, year, salaries, bonuses, config, logo, mode, selectedMonths, includeBonus, showSummary) {
...
function doRender() {
  ...
  const showSummary = d.params.showSummary === true;
  const offsetX = d.params.offsetX || 0;
  const offsetY = d.params.offsetY || 0;
  
  const html = d.params.emps.map(emp => renderPage(emp, d.params.year, d.salaries, d.bonuses, d.config, d.logo, mode, selectedMonths, includeBonus, showSummary)).join('');
